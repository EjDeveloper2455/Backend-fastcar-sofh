...
-- Dumping data for table bd_ej_soft.tbl_cobertura: ~0 rows (approximately)
...
-- Dumping data for table bd_ej_soft.tbl_vehiculo_extra: ~0 rows (approximately)
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `view_vehiculo` AS select `v`.`vehiculo_vin` AS `vin`,`v`.`vehiculo_placa` AS `placa`,`marca`.`marca_nombre` AS `marca`,`m`.`modelo_descripcion` AS `modelo` from ((`tbl_vehiculo` `v` join `tbl_modelo` `m` on((`v`.`modelo_id` = `m`.`modelo_id`))) join `tbl_marca` `marca` on((`m`.`marca_id` = `marca`.`marca_id`))) order by `v`.`vehiculo_vin`;
